...
# ---------------------
# Main API (NOW USES IMAGE ARRAY)
# ---------------------
def classify_image(img):

    # 🔥 SAFETY: resize again (double safety)
    if img.shape[0] > 500:
        scale = 500 / img.shape[0]
        img = cv2.resize(img, (int(img.shape[1] * scale), 500))
...
# 🔥 Optimized MTCNN (important)
detector = MTCNN(min_face_size=40)

# Fallback Haar
face_cascade = cv2.CascadeClassifier(
    str(OPENCV_DIR / "haarcascade_frontalface_default.xml")
)
